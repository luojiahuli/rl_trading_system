...
| 当前回撤 | -9.55% | 距峰值回落幅度 |
| 回撤状态 | normal | 硬止损-8%/软预警-5% |
| Kelly 仓位比例 | 25% | 保守仓位控制 |

### 分析结论

- **趋势跟踪**在本轮含明显趋势的合成数据中表现最优（+29.67%, Sharpe 1.669），捕捉到了上升段的均线金叉信号
- **均值回归**小幅盈利（+3.65%），在震荡阶段有正贡献但在趋势段表现平平
- **突破策略**未触发交易，说明合成数据的 Bollinger 突破条件较苛刻，需要更强的价格波动
- **动量策略**亏损（-13.52%），5日动量在趋势反转时产生了错误信号
- **最佳适配**: 当前震荡市环境下，均值回归策略可能比趋势跟踪更适合
